...
IMUPreintegrator::IMUPreintegrator(const IMUPreintegrator& pre):
    ...
    _delta_time(pre._delta_time),_rk4Preint(pre._rk4Preint)
{
}
...
void IMUPreintegrator::update(const Vector3d& omega, const Vector3d& acc, const double& dt)
{
    ...
    _delta_time += dt;
    Matrix<double,9,9> NoiseCov;
    NoiseCov.setZero();
    NoiseCov.block<3,3>(0,0)= IMUData::getGyrMeasCov();
    NoiseCov.block<3,3>(3,3)= IMUData::getAccMeasCov();
    _rk4Preint.IntegrateOnce(omega, acc, dt, NoiseCov);
